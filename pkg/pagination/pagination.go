...
func New(ctx *gin.Context, db *gorm.DB, baseURL string, PerPage int) *Pagination {
	// 默认每页数量
	if PerPage <= 0 {
		PerPage = config.LoadConfig().Pagination.PerPage
	}
	...
	if strings.Contains(baseURL, "?") {
		p.BaseURL = baseURL + "&" + config.LoadConfig().Pagination.UrlQuery + "="
	} else {
		p.BaseURL = baseURL + "?" + config.LoadConfig().Pagination.UrlQuery + "="
	}
...
func (p Pagination) GetPageFromRequest(r *gin.Context) int {
	page := r.Request.URL.Query().Get(config.LoadConfig().Pagination.UrlQuery)
